...
export function ShareFolderDialog({
  ...
  const loadData = async () => {
    try {
      ...
      const [usersResponse, sharesData] = await Promise.all([
        getUserList(),
        getFolderShares(ownerId, folderName),
      ]);
  ...
  const handleShare = async () => {
    ...
    try {
      setIsLoading(true);
      await shareFolder(folderName, ownerId, selectedUserId);
      toast({
        title: "Success",
      ...
      toast({
        variant: "destructive",
        title: "Error",
        description: error?.response?.data?.error || "Failed to share folder",
      });
    } finally {
      setIsLoading(false);
    }
  ...
  const handleRevoke = async (shareId: number) => {
    try {
      setIsLoading(true);
      await revokeFolderShare(shareId);
      toast({
        title: "Success",
  ...
  const availableUsers = users.filter(
    (user) =>
      user.userId !== ownerId &&
      !shares.some((share) => share.sharedWithUserId === user.userId)
  );
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle>Share Folder: {folderName || "(No Folder)"}</DialogTitle>
        </DialogHeader>
